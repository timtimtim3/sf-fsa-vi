...
def main(cfg: DictConfig) -> None:
    ...
    train_env = gym.make(env_name, max_wind = 2)
    eval_env = gym.make(env_name, max_wind = 2)
    ...
    for oidx, option in enumerate(lof.options):
        ...
        data = [[str(state) + train_env.MAP[state], error] for (state, error) in zip(train_env.coords_to_state.keys(), errors)]
        table = wandb.Table(data=data, columns = ["state", "error"])

        wandb.log({f"option_learning/option_{oidx}/final_errors_per_state" :  wandb.plot.bar(table, "state", "error", title="Custom Bar Chart")})
